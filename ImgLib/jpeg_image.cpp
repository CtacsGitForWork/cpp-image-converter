...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    /* Шаг 2: устанавливаем выходной файл */
#ifdef _MSC_VER
    ...
    /* Шаг 4: устанавливаем параметры кодирования по умолчанию */
    jpeg_set_defaults(&cinfo);

    /* Шаг 5: начинаем кодирование */
    jpeg_start_compress(&cinfo, TRUE);

    // подготавливаем буфер для строки изображения
    row_stride = image.GetWidth() * 3;
    std::vector<JSAMPLE> buffer(row_stride);

    /* Шаг 6: while (кодируем строки изображения) */
    while (cinfo.next_scanline < cinfo.image_height) {
        const Color* line = image.GetLine(cinfo.next_scanline);

        // Конвертируем строку изображения в формат JPEG   
    ...
    /* Шаг 7: завершаем кодирование */
    jpeg_finish_compress(&cinfo);

    /* Шаг 8: освобождаем ресурсы */
    fclose(outfile);
